        ...
        <AnimatePresence>
          {products.map((product) => (
            ...
            >
              ...
              <div
                className={`aspect-square relative overflow-hidden bg-gray-50 dark:bg-slate-800 transition-colors ${product.stock > 0 ? 'cursor-pointer' : 'cursor-not-allowed'}`}
              ...
              >
                ...
                <div className="absolute bottom-2 right-2 z-20">
                  <div className={`px-1.5 py-0.5 rounded text-[9px] font-bold shadow-sm backdrop-blur-md border ${product.stock <= 0
                    ? 'bg-red-100/90 text-red-700 border-red-200'
                    : 'bg-white/90 dark:bg-slate-900/90 text-gray-800 dark:text-white border-white/20'
                    }`}>
                    {product.stock} {product.unitType === 'kg' ? 'kg' : 'un.'}
                  </div>
                </div>
              </div>
              ...
              <div className="p-2.5 flex flex-col flex-1">
                ...
                <div className="mt-auto flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`nav-button-premium w-8 h-8 rounded-xl flex items-center justify-center transition-all shadow-sm active:scale-90 ${product.stock > 0
                      ? 'bg-emerald-600 text-white hover:bg-emerald-500'
                      : 'bg-gray-100 dark:bg-slate-800 text-gray-400 dark:text-gray-600 cursor-not-allowed'
                      }`}
                  >
                    {product.stock > 0 ? <Plus size={16} strokeWidth={3} /> : <ShoppingBag size={14} />}
                  </button>
                </div>
              </div>
            </motion.div>
